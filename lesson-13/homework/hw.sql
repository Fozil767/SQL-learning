...
--Xodimlar jadvali yordamida emp_id + first_name + familiya degan ma'noni anglatuvchi 
--"100-Steven King"ni chiqaradigan so'rovni yozishingiz kerak.
select EMPLOYEE_ID,FIRST_NAME,LAST_NAME from Employees where FIRST_NAME = 'Steven' and LAST_NAME = 'King'
...
--Xodimlar jadvalidagi telefon_raqamining qismini yangilang, 
--telefon raqami ichidagi "124" pastki qatori "999" bilan almashtiriladi.
UPDATE employees
...
SELECT 
    first_name AS "First Name", len(first_name) AS "Name Length"
FROM employees
...
--5)Write a query to retrieve the year and the highest value from the columns Max1, Max2, and Max3 for each row in the TestMax table
	SELECT Year1, greatest(Max1, Max2, Max3) as kattasi
from TestMax;
...
select * from cinema where id != 2 and id != 4 and description != 'boring'
...
--Agar birinchi ustun null bo'lsa, keyingisiga o'ting va hokazo.
