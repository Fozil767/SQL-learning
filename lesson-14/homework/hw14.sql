...
(3, '2015-01-03', 20),
(4, '2015-01-04', 30);
select  Id from (select id, Temperature,lag(Temperature) over (order by RecordDate) as PrevTemp
    from weather
) as TempWithLag
...
Insert Into MultipleVals values
 (1,'a,b,c')
,(2,'x,y,z')
select Id, parsename(REPLACE(Vals, ',', '.'), 3) as Name,
    parsename(REPLACE(Vals, ',', '.'), 2) as Surname
from MultipleVals;
...
--Vals.(GetIntegers) nomli ustunda satr boshida paydo boʻladigan butun son qiymatini chiqarish uchun SQL soʻrovini yozing.
...
select 
    Id,
    case
        when VALS IS NOT NULL AND patindex('%[0-9]%', VALS) = 1 then 
...
--Sizga ikkita jadval beriladi: p1 va p2. Ushbu jadvallarni id ustuniga qo'shing.
--Qachonki, p1.code qiymati 0 bo'lsa, uni p2.code qiymati bilan almashtiring.(p1,p2)
...
create table Employees5 (EMPLOYEE_ID int, FIRST_NAME varchar(50), LAST_NAME varchar(50),EMAIL varchar(255), PHONE_NUMBER varchar(50),HIRE_DATE date,JOB_ID varchar(50),SALARY float,COMMISSION_PCT float, MANAGER_ID int, DEPARTMENT_ID int)
